  ...
  do
    ...
    do
		...
		then
			...
			if [ $? -eq 0 ]
			then
				LogMsg " Partition disk $j is already mounted: Success"
				UpdateSummary " Partition disk $j is already mounted: Success"
				continue
			fi
			mount $j /mnt/$mnt_name
			if [ $? -eq 0 ]
			then
				LogMsg " Partitioning disk $j : Success"
				UpdateSummary " Partitioning disk $j : Success"
			else
				UpdateTestState "TestAborted"
				LogMsg " Partitioning disk $j : Failed"
				UpdateSummary " Partitioning disk $j : Failed"
				exit 1
			fi
		fi
    done
